...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    Q = np.vstack([X.ravel(), Y.ravel(), Z.ravel()]).T # grid points in 3D space
    tree = KDTree(points) # build kd-tree from the point cloud
    _, indices = tree.query(Q, k=1) # find the nearest point in the point cloud for each grid point
    indices = indices.flatten()
    closest_points = points[indices] # closest points in the point cloud to the grid points
    closest_normals = normals[indices] # normals at the closest points
    vectors = Q - closest_points # vectors from the closest points to the grid points
    signed_distances = np.sum(vectors * closest_normals, axis=1) # signed distances to the tangent plane at the closest points
    IF = signed_distances.reshape(X.shape) # reshape the signed distances to the grid shape
