...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        # Compute L1 loss between predicted and ground truth SDF values
        loss = torch.abs(pred_sdf_tensor - gt_sdf_tensor).mean()
        # Backward pass
        loss.backward()
...
def val(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        with torch.no_grad():
            ...
            # Clamp both predicted and ground truth SDF values
            pred_sdf_clamped = torch.clamp(pred_sdf_tensor, -args.clamping_distance, args.clamping_distance)
            gt_sdf_clamped = torch.clamp(gt_sdf_tensor, -args.clamping_distance, args.clamping_distance)

            # Compute L1 loss between clamped values
            loss = torch.abs(pred_sdf_clamped - gt_sdf_clamped).mean()
